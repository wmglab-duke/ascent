...
#define _style_k	*((int*)_ppvar[6]._pvoid)
...
static void nrn_alloc(Prop* _prop) {
 ...
 nrn_promote(prop_ion, 3, 0);
 	...
 	_ppvar[6]._pvoid = (void*)(&(prop_ion->dparam[0]._i)); /* iontype for k */
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 9, 10);
  ...
  hoc_register_dparam_semantics(_mechtype, 8, "diam");
 	...
 	ivoc_help("help ?1 nadifl D:/Documents/access/First_NeuronSim/MOD_Files/nadifl.mod\n");
...
_MATELM1(1, 1) *= ( PI * diam * diam / 4.0);  }
 ...
 f_flux = b_flux = 0.;
 _RHS1( 1) += (b_flux =   ( - ina / ( FARADAY ) * PI * diam * ( 1e4 ) ) );
 /*FLUX*/
 ...
 v=_v;
{
  ...
  ik = _ion_ik;
  ki = _ion_ki;
 {  sparse_thread(&_thread[_spth1]._pvoid, 2, _slist1, _dlist1, _p, &t, dt, conc, _linmat1, _ppvar, _thread, _nt);
     if (secondorder) {
    int _i;
    for (_i = 0; _i < 2; ++_i) {
