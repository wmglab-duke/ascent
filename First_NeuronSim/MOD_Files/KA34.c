...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 ka34 D:/Documents/access/First_NeuronSim/MOD_Files/KA34.mod\n");
...
double rates ( _threadargsprotocomma_ double _lVm ) {
 ...
 _lq10 = pow( 3.0 , ( ( celsius - 22.0 ) / 10.0 ) ) ;
   minf = 1.0 / ( 1.0 + exp ( - 1.0 * ( v - 28.0 - minfshift ) / 25.3 ) ) ;
   hinf = 1.0 / ( 1.0 + exp ( ( v + 29.0 - hinfshift ) / 8.0 ) ) ;
   tau_m = 0.66 + 1.0 / ( 0.13 * exp ( ( v - mtaushift ) / 13.0 ) + 0.04 * exp ( - 1.0 * ( v - mtaushift ) / 39.3 ) ) ;
   tau_h = 15.0 + 22.7 * exp ( - 1.0 * ( v - htaushift ) / 24.8 ) ;
   tau_m = tau_m / _lq10 ;
   tau_h = tau_h / _lq10 ;
