...
parser.add_argument(
    '-l', '--list', choices=['runs', 'samples', 'sims'], help='List all available indices for the specified option'
)
...
cs_parser = subparsers.add_parser(
    'clean_samples', help='Remove all files except those specified from Sample directories'
)
...
pipeline_parser.add_argument(
    'run_indices', type=int, nargs='+', help='Space separated indices to run the pipeline over'
)
pipeline_parser.add_argument(
    '-b',
    '--break-point',
...
pipeline_parser.add_argument(
    '-P', '--partial-fem', choices=["cuff_only", "nerve_only"], help="Only generate the specified geometry."
)
pipeline_parser.add_argument(
    '-E',
    '--export-behavior',
...
pipeline_parser.add_argument(
    '-e', '--endo-only-solution', action='store_true', help="Store basis solutions for endoneurial geometry ONLY"
)
pipeline_parser.add_argument(
    '-r', '--render-deform', action='store_true', help="Pop-up window will render deformation operations"
)
pipeline_parser.add_argument(
    '-S', '--auto-submit', action='store_true', help="Automatically submit fibers after each run"
)
...
prog_group.add_argument(
    '-C', '--comsol-progress-popup', action='store_true', help="Show COMSOL progress in a pop-up window"
)
...
nsims_group.add_argument(
    '-f', '--force', action='store_true', help='Import n_sims even if all thresholds are not found'
)
nsims_group.add_argument(
    '-D', '--delete-nsims', action='store_true', help='After importing delete n_sim folder from NSIM_EXPORT_PATH'
)
