...
After confirming that you are in the correct directory, the script will install the required Python packages: Pillow {cite}`clark2015pillow`, NumPy {cite}`harris2020array`, Shapely {cite}`shapely2007`, Matplotlib {cite}`Hunter:2007`, PyClipper {cite}`pyclipper2015`, pygame {cite}`pygame2011`, QuantiPhy {cite}`quantiphy2016`, OpenCV {cite}`opencv_library`, PyMunk {cite}`pymunk`, and SciPy {cite}`Virtanen2020`, pandas {cite}`reback2020pandas,mckinney-proc-scipy-2010`, OpenPyXL {cite}`OpenpyXL2020`, scikit-image {cite}`van2014scikit`,
ffmpeg {cite}`tomar2006converting`, xlsxwriter {cite}`xlsxwriter2017`, seaborn {cite}`Waskom2021`. It is crucial that all of the packages are installed successfully (check for "successfully installed" for each package). The installation script will also create a shortcut to the newly configured ASCENT Conda "environment" on your project path, which can be used for running the pipeline.
