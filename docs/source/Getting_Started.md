...
After confirming that you are in the correct directory, the script will install the required Python packages: Pillow needrefhere, NumPy needrefhere, Shapely needrefhere, Matplotlib needrefhere, PyClipper needrefhere, pygame needrefhere, QuantiPhy needrefhere, OpenCV needrefhere, PyMunk needrefhere, and SciPy needrefhere, pandas needrefhere, and OpenPyXL needrefhere. It is crucial that all of the packages are installed successfully (check for "successfully installed" for each package). The installation script will also create a shortcut to the newly configured ASCENT Conda "environment" on your project path, which can be used for running the pipeline.
