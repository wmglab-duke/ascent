...
After confirming that you are in the correct directory, the script will install the required Python packages: Pillow {cite}`clark2015pillow`, NumPy {cite}`harris2020array`, Shapely {cite}`shapely2007`, Matplotlib {cite}`Hunter:2007`, PyClipper {cite}`pyclipper2015`, pygame {cite}`pygame2011`, QuantiPhy needrefhere, OpenCV needrefhere, PyMunk needrefhere, and SciPy needrefhere, pandas needrefhere, OpenPyXL needrefhere, scikit-image needrefhere,
ffmpeg needrefhere, xlsxwriter needrefhere, seaborn needrefhere. It is crucial that all of the packages are installed successfully (check for "successfully installed" for each package). The installation script will also create a shortcut to the newly configured ASCENT Conda "environment" on your project path, which can be used for running the pipeline.
