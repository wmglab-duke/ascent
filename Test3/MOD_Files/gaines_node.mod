...
NEURON {
	...
	RANGE gnapbar, gnabar, gkbar, gl, gkfbar, ena, ek, el
...
FUNCTION vtrap12(x) {
	:Alpha n gating
	vtrap12 = anA * (x-anB) / (1 - Exp((anB-x)/anC))
}

FUNCTION vtrap13(x) { 
	:Beta n gating
	vtrap13 = bnA * (bnB-x) / (1 - Exp((x-bnB)/bnC))
}
