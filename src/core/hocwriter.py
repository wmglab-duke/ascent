...
class HocWriter(Exceptionable, Configurable, Saveable):
    ...
    def build_hoc(self, n_tsteps):
        ...
        fiber_model_info: dict = self.search(
            Config.FIBER_Z, MyelinationMode.parameters.value, fiber_model
        )

        # if myelinated
        if (
            fiber_model_info.get("neuron_flag") == 2
            and fiber_model != FiberGeometry.B_FIBER.value
        ):
            file_object.write(
        ...
        file_object.write(
            "flag_extracellular_stim = %0.0f // Set to zero for off; one for on \n" % 1
        )
        file_object.write(
            "flag_whichstim = %0.0f // Set to zero for off; one for on \n" % 0
        )
        ...
        if 'end_ap_times' in self.configs[Config.SIM.value]['saving'].keys():
            ...
            if not ((1 >= loc_min >= 0) and (1 >= loc_max >= 0)):
                self.throw(115)
            if any(
                [loc_min == 0, loc_min == 1, loc_max == 0, loc_max == 1]
            ) and fiber_model_info.get("passive_end_nodes"):
                self.throw(116)
        ...
        try:
            protocol_mode: NeuronRunMode = [
                mode
                for mode in NeuronRunMode
                if str(mode).split('.')[-1] == protocol_mode_name
            ][0]
        except:
            self.throw(135)
        ...
        if protocol_mode != NeuronRunMode.FINITE_AMPLITUDES:
            ...
            if 'ap_detect_location' not in threshold.keys():
                ...
                file_object.write(
                    ...
                    % self.search(
                        Config.SIM, "protocol", "threshold", "ap_detect_location"
                    )
                )

            bounds_search_mode_name: str = self.search(
                Config.SIM, "protocol", "bounds_search", "mode"
            )
            bounds_search_mode: SearchAmplitudeIncrementMode = [
                mode
                for mode in SearchAmplitudeIncrementMode
                if str(mode).split('.')[-1] == bounds_search_mode_name
            ][0]
            if bounds_search_mode == SearchAmplitudeIncrementMode.PERCENT_INCREMENT:
                ...
                step: float = self.search(
                    Config.SIM, "protocol", "bounds_search", "step"
                )
                file_object.write("\nrel_increment = %0.4f\n" % (step / 100))
            ...
            termination_criteria_mode: TerminationCriteriaMode = [
                mode
                for mode in TerminationCriteriaMode
                if str(mode).split('.')[-1] == termination_criteria_mode_name
            ][0]
            if termination_criteria_mode == TerminationCriteriaMode.ABSOLUTE_DIFFERENCE:
                ...
                res: float = self.search(
                    Config.SIM, "protocol", "termination_criteria", "tolerance"
                )
                file_object.write("\nabs_thresh_resoln = %0.4f\n" % res)
            elif (
                termination_criteria_mode == TerminationCriteriaMode.PERCENT_DIFFERENCE
            ):
                termination_flag = TerminationCriteriaMode.PERCENT_DIFFERENCE.value
                res: float = self.search(
                    Config.SIM, "protocol", "termination_criteria", "percent"
                )
                file_object.write("\nrel_thresh_resoln = %0.4f\n" % (res / 100))
            file_object.write("termination_flag = %0.0f // \n" % termination_flag)

            max_iter = self.search(Config.SIM, "protocol", "bounds_search").get(
                "max_steps", 100
            )
            ...
            file_object.write(
                "\nap_thresh = %0.0f\n" % 1000
            )  # arbitrarily high, not ever used
            for amp_ind in range(num_amps):
                file_object.write(
                    "stimamp_values.x[%0.0f] = %0.4f\n" % (amp_ind, amps[amp_ind])
                )
        ...
        for time_ind in range(n_checktimes):
            file_object.write(
                "checktime_values_ms.x[{}] = {} \n".format(
                    time_ind, checktimes[time_ind]
                )
            )
